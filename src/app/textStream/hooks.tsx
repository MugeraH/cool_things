...
export let greatGatsbyFull = `In my younger and more vulnerable years my father gave me some advice that I’ve been turning over in my mind ever since.
...
export function getNextChars(n: number, position: number) {
...
export function useAnimatedText(text: string) {
  ...
  let [cursor, setCursor] = React.useState(0);
  const [prevText, setPrevText] = React.useState(text);
  const [isSameText, setIsSameText] = React.useState(true);
  if (prevText !== text) {
    setPrevText(text);
  ...
  React.useEffect(() => {
    ...
    // type: "spring",
    // bounce: 0.8,
    let controls = animate(animatedCursor, text.split(" ").length, {
      duration: 2,
      ease: "linear",
  ...
  }, [text, animatedCursor, isSameText]);
